...
# bind all county demographic data together
...
mdi2 <- mdi %>% 
  mutate(GEO.id2 = ifelse(nchar(county) == 4, paste0("0", county), as.character(county))) %>% 
  mutate(mdi_rate = 100*as.numeric(`MDI rate`)) # introduces NAs for "X"

table(is.na(mdi2$mdi_rate)) # 3 missing due to data collection issues
...
# I think this is used to calculate normalized wastewater flow (mil L/km2)
...
src_wwtp1 <- src_wwtp %>% mutate(`Existing Total Flow (Mgal/d` = replace_na(0))
...
# Quality check source dataframes -----------------------------------------

# check matches
setdiff(cn14$geography, src_mfta3$geography)
...
#cn14.2 <- cn14.1 %>% mutate_if(is.numeric, ~ifelse(is.na(.), 0, .))
