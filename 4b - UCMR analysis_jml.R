...
## JML check (3/23):
 # it's possible sample numbers are undercountedfor a few systems (e.g., MI0004455)
 # but this may depend on how you define a sample: either combo of PWSID-SamplePoint-Date or counting when an analyte is measured once
 # when there are multiple potential samples on one day. this is why the following contains list-cols:
  jml_check_num_samp <- ucmr3_targetsamples %>%
    pivot_wider(id_cols = c(PWSID, SamplePointName, CollectionDate),
                names_from = Contaminant,
                values_from = AnalyticalResultsValue)
  
  # this may not be a big problem. it seems like counting sample ID as well would count these:
  jml_check_num_samp2 <- ucmr3_targetsamples %>%
    group_by(PWSID, SamplePointName, CollectionDate, sys.sampid) %>% 
    summarise(n = n()) %>% 
    group_by(PWSID) %>%
    summarise(n_samples_jl = n())              
## end of check
  
  n_samples_dat %>% left_join(jml_check_num_samp2, by = c("PWSID"))
...
crude2df <- function(mod){
  ...
  mod %>%
    ...
    mutate(perc.change = round((exp(estimate)-1)*100, digits = 1),
...
mod2df <- function(mod){
  ...
  mod %>% mutate(conv = ifelse(round((exp(estimate + 1.96*std.error)-1)*100, digits = 1)>9999, 1, 0), 
                 ...
                 result = ifelse(conv == 1, NA, paste0("", as.character(format(perc.change, nsmall = 1, digits = 2)), " (",
                                as.character(format(lo.ci, nsmall = 1, digits = 2)),
                                ", ", as.character(format(hi.ci, nsmall = 1, digits = 2)), ")")),
                 p.value = ifelse(conv == 1, NA, case_when(p.value < 0.001 ~ paste0("<0.001"), 
                                     TRUE ~ paste0(round(p.value, 4))))) %>%
...
clean_output <- function(mod){
  mod %>% 
    mutate(term = case_when(term == "perc_hisp_any" ~ "Percent Hispanic",
                            ...
                            term %in% c("as.factor(Size)S", "SizeS")  ~ "Small systems (ref = Large)",
                            term %in% c("as.factor(source_type)SW", "source_typeSW")  ~ "Surface water systems (ref = GW)",
                            term %in% c("as.factor(source_type)MIX", "source_typeMIX") ~ "MIX systems (ref = GW)",
                            ...
                            term == "sesval" & ("sesvar" %in% colnames(mod) && sesvar == "perc_hmown") ~ "Percent Homeownership",
    ...
    mutate(term = factor(term, levels = c("Percent Hispanic",
                                          ...
                                          "Small systems (ref = Large)",
                                          "Surface water systems (ref = GW)",
                                          "MIX systems (ref = GW)",
...
## JML check (3/23)
  # looks good after a few spot checks! one comment:
  # i think the fitted probability warning is happening with n_samples.
  # may want to consider logging it shrink the distribution of n_samples a bit
  # then the coefficient is interpretable as change in odds of detection for 
  # a doubling of the number of samples (also seems to remove warning)
  # feel free to remove my checks once they're addressed also!
...
# write.csv(cladjglmermods,
#           paste0("results/output/all_adjMEEs_", Sys.Date(), ".csv"))
...
## the difference in interpretations.
  ...
  ### JML check (3/23): I think this is expected! rescaling (i.e., by default, subtracting mean and dividing by SD) 
  # the continuous variables shouldn't change significance since we're only shifting everything
  # (I checked that this is the case here, which is good), but it changes interpretation.
  # e.g., in adjglmermods, we have a one-percentage-point change in x is associated with % change in odds of outcome
  # in cladjglmermods, we have a standard deviation change in x is associated with a % change in odds of outcome
  # rescaling is sometimes necessary/you'll get warnings with mixed models bc 
  # otherwise the intercepts become difficult for your computer to estimate
    # y ~ x will yield an estimated intercept corresponding to y when x = 0 (but sometimes x = 0 is meaningless/rare)
    # while y ~ scale(x) will yield an intercept when x is equal to the (sample) mean value of x
  # demonstrating this:
  m1 <- lme4::glmer("hlvlchem ~ perc_hisp_any + perc_black_nohisp + 
                                propurban + as.factor(Size) + 
                                as.factor(source_type) + 
                                WWTP_ML_km2 + n_samples + mdi + bin_TRI + (1|stateabbr)",
                            data = processed, family = "binomial")
  m1.summary <- summary(m1)
  unscaled.coefficient <- m1.summary$coefficients["perc_hisp_any", "Estimate"]
  exp(unscaled.coefficient) # odds ratio of ~1.016 for each p.p. increase in % Hispanic/Latino
  
  sd.var <- sd(processed$perc_hisp_any) # note: sd of % Hispanic/Latino is 15.5 p.p.
  
  m1 <- lme4::glmer("hlvlchem ~ scale(perc_hisp_any) + scale(perc_black_nohisp) + 
                                scale(propurban) + as.factor(Size) + 
                                as.factor(source_type) + 
                                scale(WWTP_ML_km2) + scale(n_samples) + scale(mdi) +
                                bin_TRI + (1|stateabbr)",
                    data = processed, family = "binomial")
  m1.summary <- summary(m1)
  scaled.coefficient <- m1.summary$coefficients["scale(perc_hisp_any)", "Estimate"]
  exp(scaled.coefficient) # odds ratio of 1.28. 
  # this is equivalent to 1.016^(15.5) as expected, which is (roughly due to rounding error) same as:
  exp(unscaled.coefficient * sd.var)
  ### end of check
...
ucmrdf.test_evrdet_corr <- ucmrdf.mod[ucmrdf.mod$contam.pfas=="evrdet",] %>%
  ...
  select("PWSID", "contam.pfas", "perc_hisp_any", "propurban", "mdi", "perc_uninsur", "perc_pov_ppl", "pov_status_co",
  ...
  rename("Percent \nHispanic" = "perc_hisp_any",
         ...
         "Percent \nPoverty" = "perc_pov_ppl",
         "MDI Rate" = "mdi", 
         "Percent \nUninsured" = "perc_uninsur", 
         "TRI facility (any)" = bin_TRI,
         ...
         ">20% of county \nis below pov line" = pov_status_co,
         ...
         "N samples" = n_samples, 
         "Size" = Size, 
         "GW" = GW_bi, "SW" = SW_bi, "MIX" = MIX_bi) %>%
  select(-source_type)

cor.matrix_evrdet <- cor(ucmrdf.test_evrdet_corr[,c(3:27)], method = "spearman",
                         use = "complete.obs")

p.mat <- cor.mtest(ucmrdf.test_evrdet_corr[,c(3:27)])
